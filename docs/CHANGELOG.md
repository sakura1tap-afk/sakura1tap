...
- 首屏改用轻量电影式入口，不再进入页面前加载未被主页使用的 3D 模型和两套 Live2D 资源。
- 手机、触控设备和减少动态效果的设备跳过持续 WebGL 场景，自动使用图片降级背景。
- 静态入口与场景背景增加低成本呼吸动效，降级后不再像画面冻结。
- 移除大面积高频 SVG 噪点重绘，降低 GPU 合成压力，并修复入口样式中的无效换行字符。
- 桌面 WebGL 渲染像素倍率上限由 1.5 降到 1.25，在尽量保留清晰度的同时减少每帧填充量。
