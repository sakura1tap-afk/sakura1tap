...
- 软件 WebGL（SwiftShader、llvmpipe 等）不再运行全屏着色器，自动切换到同一场景图的低成本呼吸背景，避免拖死页面滚动。
- 硬件 WebGL 继续保留指针涟漪、拖拽、章节混合和呼吸效果，渲染上限调整为 40 FPS。
- 移除每个像素每帧计算的动态噪点，并停止在每一帧读取布局尺寸；窗口尺寸变化时才重设画布。
- WebGL 上下文丢失时立即显示动态兜底背景，避免画面永久停在最后一帧。

### 修复：保留双 Live2D 的入口卡顿治理

- 恢复原有双 Live2D 入口；模型在电影式背景上渐进加载，入口按钮无需等待模型即可使用。
- 移除入口对未被主页使用的 3D GLB 预加载依赖，避免无意义的下载和 15 秒等待。
- 五张 Live2D 纹理改用 WebP，运行时纹理由约 20.3 MB 降至约 2.0 MB；原始 PNG 留在仓库用于维护，不影响现有效果。
- 静态背景增加低成本呼吸动效，并移除大面积高频 SVG 噪点重绘，降低 GPU 合成压力。
- Live2D 像素倍率上限由 2 降到 1.5，首页 WebGL 上限由 1.5 降到 1.25，减少高分屏每帧填充量。
- 修复入口样式中的无效换行字符；不支持 WebGL 的浏览器仍自动使用轻量入口。

### 维护：重资源缓存与 Live2D 完整性检查

- GitHub `main` 的 532 个 `public/` 文件已完整复核，生产构建通过，资源并未从仓库丢失。
- 哈希 JS/CSS 改为一年不可变缓存；图片、视频、模型、Live2D 与 runtime 改为一天浏览器缓存并允许七天后台复验。
- `/api/health` 增加两套 Live2D 模型配置与代表纹理，关键资源检查由 8 项扩展为 12 项。
- 保留 HTML `no-store` 与缺失静态文件真实 404，避免旧 HTML 引用已删除分包或把 HTML 误解析为资源。

## 2026-09-07

### 维护：静态资源健康检查与失败恢复

- 线上复核 `/`、`/play`、`/play/reaction`、`/lab`、排行榜、首页场景图和四段实验室视频，当前生产资源仍然存在且可加载。
- Worker 不再把缺失的 `.js`、图片、视频、模型等资源回退成 `index.html`；资源缺失现在返回真实 404 与 `nosniff`，避免表现为难以理解的 MIME 错误或黑块。
- `/api/health` 从仅检查 D1 扩展为同时探测入口背景、场景图、实验室视频、GLB 和 Live2D runtime，并列出失败路径。
- 动效实验室的视频窗口增加加载失败状态；单个片段失败不会拖垮页面，点击该窗口可以原位重新加载。
- 新增 `docs/specs/ASSET_RELIABILITY.md`，记录诊断结论、资源边界和后续维护入口。

## 2026-07-18

### 修复：零件力场 V2 外力累积、纵深乱飞与变色难命中

- 定位到 Rapier 用户力会持续保留，旧循环重复 `addForce` / `addTorque` 却未清零，导致零件无限加速和旋转。
- 每个 60Hz 物理步改为先清空用户力与扭矩，再计算当前力场；瞬时点击与冲击仍使用 impulse。
- 世界重力锁定为屏幕 Y 轴向下；X/Y 使用平面中心弹簧，Z 轴改为窄范围强阻尼回中，删除纵深主导运动。
- 初始纵深和角速度收窄，提高角阻尼，并将线速度 / 角速度保护上限降至 6.5 / 5。
- 确认点击变色代码此前已经上线；本次补充屏幕空间拾取容错、确定性色板轮换和缩放脉冲，使反馈可见且易命中。
- 完成严格 TypeScript、生产分包构建与 60 秒 Rapier 稳定性回归，测试体最终速度接近零且 Z 轴回到画面平面。

## 2026-07-18

### 升级：零件力场 V2 刚体物理与变色拖拽

- 将第一版手写球形近似碰撞替换为 Rapier 3D 刚体世界，Three.js 回归纯渲染职责。
- 加入固定 60Hz 物理步、质量、惯性、摩擦、弹性、角速度、弱向下重力和距离相关的中心引力，解决零件轻飘与碰撞失真。
- 高性能桌面档从 26 个提升到 56 个零件，并扩展为 10 类几何；圆环和扭结使用多球复合碰撞体。
- 点击零件会在主题色板中变色并获得角冲量；按住可用阻尼弹簧力拖动；点击空白继续释放范围冲击。
- 根据视口、CPU 核数、设备内存和 reduced-motion 将数量降到 40 / 28 / 18；持续低帧率时自动将 DPR 降为 1。
- Rapier 保持在“仿 Lusion”动态分包内，不增加首页首包；退出栏目时释放物理世界和全部 WebGL 资源。
- 更新 `docs/specs/MOTION_LAB_LUSION_STUDY.md`，记录 V2 力场结构、交互优先级、性能门槛与后续调参范围。

## 2026-07-18

### 新增：动效实验室“仿 Lusion”栏目与零件力场

- 保留原有四窗口放映场，在动效实验室增加独立“仿 Lusion”栏目入口。
- 新栏目使用动态分包，进入后才加载 Three.js 交互实验，不增加主页与功能空间首包负担。
- 完成 01 / 零件力场：26 个程序化金属、陶瓷和亮面零件持续受中心吸引、旋转流场、阻尼、边界与互相碰撞控制。
- 鼠标映射到三维世界平面，靠近零件产生排斥冲量；点击可释放一次扩大冲击，并支持“重新散开”。
- 窄屏与 reduced-motion 降低零件数量和 DPR；页面隐藏时暂停更新，离开栏目完整释放 WebGL 资源。
- WebGL 初始化失败或 context lost 时显示可返回的降级界面，避免再次出现永久黑屏。
- 新增 `docs/specs/MOTION_LAB_LUSION_STUDY.md`，记录参考边界、实现结构、性能策略和下一阶段滚动航行方向。

## 2026-07-18

### 修复：首页章节隐形点击热区

- 定位到功能章节在 GSAP 淡出后仍保留 `pointer-events: auto`，导致互动章节空白区域可误触 `/play` 或 `/lab`。
- 新增统一章节交互同步：只有当前章节可交互，其他章节自动设置 `inert` 与 `aria-hidden`。
- CSS 以 `data-scene` 默认封锁非当前章节的入口，避免脚本初始化前出现短暂透明热区。
- 同步覆盖首页“开始探索”、功能入口以及继续章节操作，键盘 Tab 也不会再聚焦隐藏控件。
- TypeScript 定向检查通过；部署后验证互动章节隐藏入口不再参与命中测试。
...
- 将用户输入的排行榜 ID 设为唯一身份，同一 ID 跨浏览器也只对应一条榜单记录。
- 新成绩只有平均反应时间更快时才更新；较慢或相同成绩不修改成绩、五次明细或更新时间。
- API 新增 `improved`、`isNew` 和 `bestAverageMs`，完成页分别提示首次上榜、刷新最佳或未超过最佳。
- 增加排行榜 ID 唯一索引；SQLite 验证 202ms → 250ms 保持 202ms，随后 190ms 才更新。
...
- Wrangler 声明 `REACTION_DB` 并使用官方自动配置，避免真实部署因缺少既有数据库 UUID 而阻塞；配置通过 Wrangler 4 dry-run。
